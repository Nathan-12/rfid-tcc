  ...
  <div id="media">
    <div class="layout row">
      <v-layout row wrap>
        <v-flex v-for="item in lista" :key="item.id" xs12 sm6 md3>
          <v-card class="mx-auto">
            <v-icon id="icone" color="primary" class="centralizar mb-1 opct opacity-20" size="150">far fa-file-pdf</v-icon>
            <v-tooltip top>
              <template v-slot:activator="{ on }">
                <v-card-title>
                  <div v-on="on" class="centralizar text-truncate font-weight-bold text-uppercase">{{item.nome}}</div>
                </v-card-title>
              </template>
              <span>{{item.nome}}</span>
            </v-tooltip>
            ...
            <v-tooltip top>
              <template v-slot:activator="{ on }">
            <v-card-title>
              <div v-if="item.observacao != ''" v-on="on" class="font-weight-regular centralizar text-truncate">{{item.observacao}}</div>
              <div class="centralizar" v-else>-</div>
            </v-card-title>
            </template>
      ...
      <div v-if="lista.length == 0">
        <span id="listaVazia">Nenhum arquivo adicionado</span>
      </div>
