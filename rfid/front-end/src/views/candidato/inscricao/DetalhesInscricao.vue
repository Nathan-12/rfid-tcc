  ...
  <v-container fluid grid-list-xl>
            ...
            <v-tooltip top>
              <template v-slot:activator="{ on }">
                ...
                >
                  ...
                  <input id="files" ref="files" type="file" accept="application/pdf" style="display:none" v-on:change="selecionarArquivos()" />
                </v-btn>
              </template>
              <span>Adicionar Arquivo</span>
            </v-tooltip>
            <media :lista="listaDeArquivos" :inscricaoId="inscricaoid"></media>
          </div>
        </v-widget>
      </v-flex>
    </v-layout>
    <v-dialog v-model="dialog" persistent max-width="500">
      <v-card>
        ...
        <v-card-title v-if="file!=null"><div class="text-truncate">Nome do arquivo: {{ file[0].name }}</div></v-card-title>
        <v-card-title>
          <v-textarea id="cardObservacao"
                v-model="observacao"
                label="Observação"
          ></v-textarea>
...
export default {
  ...
  methods: {
    ...
    enviarArquivos() {
      ...
      axios.post("/inscricoes/" + this.inscricaoid, formData)
        .then(res =>{
          if(((this.file[0].name).substr(-4) == ".pdf") || (this.file[0].type == "application/pdf")){
